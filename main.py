...
###################################################################################
#  pengecekan pretrained model haarscascade
# AWAL AI
...
# pengambilan foto
def TakeImages():
    # memanggil fungsi cek pretrained model
    check_haarcascadefile()  
    columns = ['SERIAL NO.', '', 'ID', '', 'NAME', '', 'UNIVERSITAS', '', 'JURUSAN', '']
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while (True):
            ret, img = cam.read()
            #  konversi foto dari bgr ke grayscale
        ...
        # memulai menulis informasi wajah ke file csv
...
def TrainImages():
    # pengecekan file haarscascade
    check_haarcascadefile()
    # membuat folder TrainingImageLabel
    assure_path_exists("TrainingImageLabel/")
    recognizer = cv2.face_LBPHFaceRecognizer.create()
    harcascadePath = "haarcascade_frontalface_default.xml" # PEMANGGILAN PRETRAINED MODEL COKKKKK
    detector = cv2.CascadeClassifier(harcascadePath) # PEMANGGILAN PRETRAIN
    ...
    try:
        ...
        return
    # saving model
    recognizer.save("TrainingImageLabel\Trainner.yml") # SAVE MODEL
...
############################################################################################
# AWAL INFERENCES/BACKEND
# mengambil informasi dari foto dan csv
...
def TrackImages():
    check_haarcascadefile()
    # MENGECEK FILE ATTENDANCE / KEHADIRAN
    assure_path_exists("Attendance/") 
    # MENGECEK FILE DETAIL PELAJAR / WAJAH
    assure_path_exists("StudentDetails/")
    ...
    i = 0
    j = 0
    # MEMBUAT OBJEK PENGENAL WAJAH DENGAN ALGORITMA LBPH (LOCAL BINARY PATTERN HISTOGRAM)
    recognizer = cv2.face.LBPHFaceRecognizer_create()  # cv2.createLBPHFaceRecognizer()
    # INI BAGIAN PENTING COOKKKKKK
    # BUAT NGECEK APAKAH MODELNYA ADA?
    # KALAU ADA MULAI BACA MODEL DENGAN VARIABEL RECOGNIZER
    ...
    if exists3:
        ...
        return
    harcascadePath = "haarcascade_frontalface_default.xml"
    faceCascade = cv2.CascadeClassifier(harcascadePath) # INI BAGIAN PENTING COOKKK

    cam = cv2.VideoCapture(0) # BUKA KAMERA INI COKKKKKK
    font = cv2.FONT_HERSHEY_SIMPLEX # INI BUAT IMPORT FONT
    ...
    if exists1:
        ...
        window.destroy()
    while True:
        # INI BUAT BUKA KAMERA COKKK
        ret, im = cam.read()
        # KONVERSI GAMBAR DARI BGR KE GRAYSCALE
        ...
        for (x, y, w, h) in faces:
            ...
            # JIKA KONFIDENTIAL KURANG DARI 50, MAKA WAJAH DIKENALI
            if conf < 50:
                ts = time.time()
    ...
    # INI BUAT MEMASUKKAN DATA BUAT ATTENDANCE COKKK
...
message3 = tk.Label(window, text="Absensi Magang Mahasiswa" ,fg="white",bg="#DC143C" ,width=55 ,height=1,font=('comic', 29, ' bold '))
message3.place(x=10, y=10)

frame3 = tk.Frame(window, bg="#DC143C")
frame3.place(relx=0.52, rely=0.09, relwidth=0.09, relheight=0.07)

frame4 = tk.Frame(window, bg="#DC143C")
frame4.place(relx=0.36, rely=0.09, relwidth=0.16, relheight=0.07)

datef = tk.Label(frame4, text = day+"-"+mont[month]+"-"+year+"  |  ", fg="white",bg="#DC143C" ,width=55 ,height=1,font=('comic', 22, ' bold '))
datef.pack(fill='both',expand=1)

clock = tk.Label(frame3,fg="white",bg="#DC143C" ,width=55 ,height=1,font=('comic', 22, ' bold '))
clock.pack(fill='both',expand=1)
tick()

head2 = tk.Label(frame2, text="                                 Absen Disini                       ", fg="black",bg="#00fcca" ,font=('comic', 17, ' bold ') )
head2.grid(row=0,column=0)

head1 = tk.Label(frame1, text="                                   Registrasi                             ", fg="black",bg="#00fcca" ,font=('comic', 17, ' bold ') )
head1.place(x=0,y=0)

lbl = tk.Label(frame2, text="NIM",width=20  ,height=1  ,fg="black"  ,bg="#c79cff" ,font=('comic', 17, ' bold ') )
lbl.place(x=80, y=55)

txt = tk.Entry(frame2,width=32 ,fg="black",font=('comic', 15, ' bold '))
txt.place(x=30, y=88)

lbl2 = tk.Label(frame2, text="Nama",width=20  ,fg="black"  ,bg="#c79cff" ,font=('comic', 17, ' bold '))
lbl2.place(x=80, y=140)

txt2 = tk.Entry(frame2,width=32 ,fg="black",font=('comic', 15, ' bold ')  )
txt2.place(x=30, y=173)

lbl3 = tk.Label(frame2, text="Universitas",width=20  ,fg="black"  ,bg="#c79cff" ,font=('comic', 17, ' bold '))
lbl3.place(x=80, y=225)

txt3 = tk.Entry(frame2,width=32 ,fg="black",font=('comic', 15, ' bold ')  )
txt3.place(x=30, y=258)

lbl4 = tk.Label(frame2, text="Jurusan",width=20  ,fg="black"  ,bg="#c79cff" ,font=('comic', 17, ' bold '))
lbl4.place(x=80, y=310)

txt4 = tk.Entry(frame2,width=32 ,fg="black",font=('comic', 15, ' bold ')  )
txt4.place(x=30, y=343)

lbl5 = tk.Label(frame2, text="Durasi Magang",width=20  ,fg="black"  ,bg="#c79cff" ,font=('comic', 17, ' bold '))
lbl5.place(x=80, y=395)

txt5 = tk.Entry(frame2,width=32 ,fg="black",font=('comic', 15, ' bold ')  )
txt5.place(x=30, y=428)
...
# bagian ini sudah mulai masuk inference
# mengecek data wajah yang telah terdaftar pada file studentdetail.csv
# LANJUT INFERENCE
exists = os.path.isfile("StudentDetails\StudentDetails.csv")
if exists:
    with open("StudentDetails\StudentDetails.csv", 'r') as csvFile1:
...
def populate_treeview():
    ...
    csv_file_path = 'Attendance/Attendance.csv'
...
clearButton2 = tk.Button(frame2, text="Clear", command=clear2  ,fg="black"  ,bg="#ff7221"  ,width=11 , activebackground = "white" ,font=('comic', 11, ' bold '))
clearButton2.place(x=335, y=257)  
clearButton2 = tk.Button(frame2, text="Clear", command=clear2  ,fg="black"  ,bg="#ff7221"  ,width=11 , activebackground = "white" ,font=('comic', 11, ' bold '))
clearButton2.place(x=335, y=342) 
clearButton2 = tk.Button(frame2, text="Clear", command=clear2  ,fg="black"  ,bg="#ff7221"  ,width=11 , activebackground = "white" ,font=('comic', 11, ' bold '))
clearButton2.place(x=335, y=427)
